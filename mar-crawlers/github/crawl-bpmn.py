    ...
    for i in range(init, end, step):
        ...
        while not finished_chunk:
            try:            
                ...
                files = g.search_code(query='bpmn extension:bpmn ' + size)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Download bpmn files from github.')
    ...
    with open(os.path.join(args.output, 'bpmn_download.csv'), 'a+', newline='') as file:
        writer = csv.writer(file)
        process(file, args.output, args.init, 1_000_000, args.step)
...
#             print("   Wrote to ", total, "...", path)
#             writer.writerow([f.path, f.name, f.download_url, f.size])
